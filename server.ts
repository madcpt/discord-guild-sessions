...
function persistChannel(name: string, channelId: string): void {
  mkdirSync(STATE_DIR, { recursive: true, mode: 0o700 })
  const map = loadSessionChannels()
  map[name] = channelId
...
async function ensureSessionChannel(channelName: string): Promise<string | null> {
  ...
  // Check the local cache first — avoids a full channel list fetch.
  const cached = lookupChannelByName(channelName)
  if (cached) {
    try {
      const guild = await client.guilds.fetch(GUILD_ID)
      const ch = await guild.channels.fetch(cached)
      if (ch && ch.type === ChannelType.GuildText) {
        process.stderr.write(`discord channel: restored session channel #${channelName} (${cached})\n`)
        registerSessionChannel(channelName, cached)
        needsSessionChannelSetup = false
        return cached
      }
    } catch {
      process.stderr.write(`discord channel: cached channel ${cached} no longer exists, re-creating\n`)
    }
  }
  ...
  if (existing) {
    ...
    registerSessionChannel(channelName, existing.id)
    needsSessionChannelSetup = false
    return existing.id
  }
  ...
  registerSessionChannel(channelName, created.id)
  needsSessionChannelSetup = false
  return created.id
}

/** Auto-register the session channel in access groups, and persist name → id mapping. */
function registerSessionChannel(channelName: string, channelId: string): void {
  sessionChannelId = channelId
  persistChannel(channelName, channelId)
